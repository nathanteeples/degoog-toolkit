...
function renderUsageCard() {
  return {
    title: "",
    html: `<div class="time-card command-result"><p class="time-card__usage">{{ t:plugin-time.usageLine1 }}</p><p class="time-card__usage">{{ t:plugin-time.usageLine2 }}</p></div>`,
  };
}
...
async function renderTimeQuery(rawInput, context, { allowDefault = false } = {}) {
  ...
  if (!resolved) {
    return {
      title: "",
      html: `<div class="time-card command-result"><p class="time-card__usage">Could not find a timezone for <strong>${escapeHtml(place)}</strong>.</p></div>`,
    };
  }
